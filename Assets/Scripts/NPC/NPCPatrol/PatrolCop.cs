...
public class PatrolCop : MonoBehaviour
{
    public float _speed;
    ...
    void Update()
    {
        ...
        {            
            _anim.SetInteger("condicao", 0);      // Stop Walking Animation and start Idle Animation
            if (_waitTime <= 0)
            {
                _randomSpot = Random.Range(0, _moveSpot.Length);
                _waitTime = _startWaitTime;
                FaceSpot();    // Chamando funcao para o NPC olhar para o spot
                _anim.SetInteger("condicao", 1);   //Play Walking Animation
            }  else
            {
